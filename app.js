...
function initHR() {
  ...
  if (isIOS()) {
    // iOS: Web Bluetooth not supported natively — show Bluefy instructions
    if (nobt) {
      ...
      nobt.innerHTML = `
        ...
        <div style="font-size:12px;font-weight:600;line-height:1.6;color:#1E1B4B;">
          ...
          To use the heart rate monitor on iOS, install
          ...
          (free on the App Store) — it's a browser that adds Bluetooth support.<br><br>
          Once installed, open the app URL in <strong>Bluefy</strong> instead of Safari,
          and the Connect button will work normally.
        </div>`;
    }
