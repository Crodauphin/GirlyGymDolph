...
function showView(name) {
  ...
  // HR widget lifecycle
  if (name === 'workout') {
    initHR(); // creates widget if needed, then show it
...
// ── INIT HR UI ────────────────────────────
function initHR() {
  // Add HR widget to DOM if not present
  if (!document.getElementById('hr-widget')) {
    const widget = document.createElement('div');
    widget.id = 'hr-widget';
    widget.innerHTML = `
      <div class="hr-bubble" id="hr-bubble" onclick="openHRPanel()">
        <div>
          <div class="hr-heart" id="hr-heart">🤍</div>
        </div>
        <div>
          <div class="hr-value" id="hr-bubble-val">--</div>
          <div class="hr-unit">BPM</div>
          <div class="hr-status" id="hr-bubble-status">tap to connect</div>
        </div>
      </div>`;
  ...
  if (!document.getElementById('hr-panel')) {
    ...
    panel.innerHTML = `
      ...
      <div class="hr-no-bt" id="hr-no-bt">
        ⚠️ Web Bluetooth is not supported in this browser.<br>
        Please use <strong>Chrome</strong> or <strong>Edge</strong> on Android or desktop.<br>
        <em>Not supported on iOS Safari.</em>
      </div>
  ...
  if (!navigator.bluetooth) {
    document.getElementById('hr-no-bt').style.display = 'block';
    document.getElementById('hr-connect-btn').style.display = 'none';
  }
}
