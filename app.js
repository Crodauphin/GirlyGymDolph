...
function showTimerAutoToast() {
  ...
  if (!toast) {
    ...
    toast.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);' +
      'background:rgba(253,232,240,.95);border:1px solid rgba(80,200,140,.4);border-radius:20px;' +
...
function drawDualLineChart(canvas, labels, weights, bfs, tall, muscles) {
  ...
  if(validW.length){
    ...
    ctx.fillStyle='rgba(80,200,140,0.1)'; ctx.fill();
    ctx.beginPath(); ctx.moveTo(xPos(0),yw(weights[0]));
    weights.forEach((v,i)=>ctx.lineTo(xPos(i),yw(v)));
    ctx.strokeStyle='#50C88C'; ctx.lineWidth=2; ctx.lineJoin='round'; ctx.stroke();
    weights.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),yw(v),3,0,Math.PI*2);ctx.fillStyle='#50C88C';ctx.fill();});
    ctx.fillStyle='rgba(80,200,140,0.7)'; ctx.font='10px Exo 2,sans-serif'; ctx.textAlign='right';
    [0,.5,1].forEach(t=>{const v=wMin+wRng*t; ctx.fillText(Math.round(v*10)/10,p.left-4,yw(v)+4);});
    ctx.fillStyle='rgba(232,121,160,0.5)'; ctx.font='10px Sofia Sans Condensed,sans-serif'; ctx.textAlign='left';
    ...
    if(muscles && muscles.some(v=>v!=null)){
      ...
      ctx.strokeStyle='#A7F3D0'; ctx.lineWidth=0.75; ctx.lineJoin='round'; ctx.setLineDash([4,3]); ctx.stroke();
      ctx.setLineDash([]);
      muscles.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),yw(v),2,0,Math.PI*2);ctx.fillStyle='#A7F3D0';ctx.fill();});
    }
  }
  ...
  if(bfs && bfs.some(v=>v!=null)){
    ...
    ctx.strokeStyle='#E879A0'; ctx.lineWidth=0.75; ctx.lineJoin='round'; ctx.stroke();
    bfs.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),ybf(v),2,0,Math.PI*2);ctx.fillStyle='#E879A0';ctx.fill();});
  ...
  let lx=p.left;
  ctx.fillStyle='#50C88C'; ctx.fillText('● Weight',lx,14); lx+=60;
  if(bfs&&bfs.some(v=>v!=null)){ctx.fillStyle='#E879A0'; ctx.fillText('● Body fat',lx,14); lx+=65;}
  if(muscles&&muscles.some(v=>v!=null)){ctx.fillStyle='#A7F3D0'; ctx.fillText('● Muscle mass',lx,14);}
}
...
function drawBarChart(canvas,labels,values,title,colors){
  ...
  // Title
  ctx.fillStyle='rgba(196,181,253,0.5)';ctx.font='10px Sofia Sans Condensed,sans-serif';ctx.textAlign='left';ctx.fillText(title,p.left,16);
}
function drawLineChart(canvas,labels,values,title,unit,lineColor,fillColor){
  ...
  ctx.fillStyle='rgba(196,181,253,0.6)';ctx.font='11px Sofia Sans Condensed,sans-serif';ctx.textAlign='left';ctx.fillText(title,p.left,16);
}
  ...
  data.forEach((section, catIdx) => {
    ...
    header.innerHTML = `
      ...
      <button onclick="addEquipmentItem(${catIdx})"
        style="background:rgba(80,200,140,.12);border:1px solid rgba(80,200,140,.3);
...
function updateHRZoneSettings() {
  ...
  if (table) {
    const rows = [
      ...
      ['Aerobic',    z.aerobic   + '–' + (z.anaerobic-1), '#A7F3D0'],
      ['Anaerobic',  z.anaerobic + '–' + (z.vo2max-1),    '#A7F3D0'],
