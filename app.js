...
function renderBodyStatsTendency() {
  ...
  function chip(label, val, invert) {
    ...
    const color = positive ? '#E879A0' : '#5B8AE8';
...
function drawDualLineChart(canvas, labels, weights, bfs, tall) {
  ...
  weights.forEach((v,i)=>{ctx.beginPath();ctx.arc(xPos(i),yw(v),3,0,Math.PI*2);ctx.fillStyle='#E879A0';ctx.fill();});

  // Left axis labels (weight)
  ctx.fillStyle='#E879A0'; ctx.font='10px Exo 2,sans-serif'; ctx.textAlign='right';
  [0,.5,1].forEach(t=>{const v=wMin+wRng*t; ctx.fillText(Math.round(v*10)/10,p.left-4,yw(v)+4);});
  ctx.fillStyle='rgba(157,78,120,0.7)'; ctx.font='10px Rajdhani,sans-serif'; ctx.textAlign='left';
  ...
  if (bfs && bfs.some(v=>v!=null)) {
    ...
    ctx.strokeStyle='#5B8AE8'; ctx.lineWidth=2; ctx.lineJoin='round'; ctx.stroke();
    bfVals.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),ybf(v),3,0,Math.PI*2);ctx.fillStyle='#5B8AE8';ctx.fill();});
    // Right axis labels (BF%)
    ctx.fillStyle='rgba(91,138,232,0.7)'; ctx.font='10px Exo 2,sans-serif'; ctx.textAlign='left';
  ...
  ctx.fillStyle='#E879A0'; ctx.fillText('● Weight',p.left,14);
  if(bfs&&bfs.some(v=>v!=null)){ctx.fillStyle='#5B8AE8'; ctx.fillText('● Body fat',p.left+60,14);}
}
...
function renderProgressChart() {
  ...
  drawLineChart(canvas,points.map(p=>p.date),points.map(p=>p.w),name,'kg','#E879A0','rgba(232,121,160,0.10)');
}
...
function renderTonnageChart() {
  ...
  updateDeloadNotices();
  const colors=weeks.map(w=>w===currentWeek?'#E879A0':'#5B8AE8');
...
function drawBarChart(canvas,labels,values,title,colors){
  ...
  // Bars
  const colorArr=Array.isArray(colors)?colors:labels.map(()=>colors||'#5B8AE8');
  values.forEach((v,i)=>{
    if(v===0) return;
    const bx=p.left+i*gap+(gap-bw)/2,bh=v/mx*ch,by=p.top+ch-bh;
    const col=colorArr[i]||'#5B8AE8';
...
function updateHRZoneSettings() {
  ...
  if (table) {
    const rows = [
      ['Resting',    '< ' + z.recovery,              '#7EC8E3'],
      ['Recovery',   z.recovery  + '–' + (z.aerobic-1),   '#C4B5FD'],
      ['Aerobic',    z.aerobic   + '–' + (z.anaerobic-1), '#50C88C'],
      ['Anaerobic',  z.anaerobic + '–' + (z.vo2max-1),    '#F9A8D4'],
      ['VO2 Max',    z.vo2max + '+',                  '#E87050'],
    ];
    table.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 16px;margin-top:4px;">' +
      rows.map(([name, range, colour]) =>
        `<span style="color:${colour};font-family:'Sofia Sans Condensed',sans-serif;font-size:15px;font-weight:700;letter-spacing:1px;">${name}</span>` +
        `<span style="color:var(--text-dim);font-family:'Sofia Sans Condensed',sans-serif;font-size:15px;">${range} bpm</span>`
      ).join('') +
      `<span style="color:var(--text-dim);font-family:'Sofia Sans Condensed',sans-serif;font-size:13px;grid-column:1/-1;margin-top:6px;">Max HR: ${z.max} bpm · based on 220 − age</span>` +
      '</div>';
  }
}
...
function updateHRPill(bpm) {
  ...
  ['zone-resting','zone-recovery','zone-aerobic','zone-anaerobic','zone-vo2max'].forEach(z =>
    pill.classList.toggle(z, z === (zoneKey||'zone-resting')));
...
// ── HR test helper (dev) — called from HR sheet test buttons ──
// Show HR test trigger on load
window.addEventListener('DOMContentLoaded', function() {
  const trig = document.getElementById('hr-test-trigger');
  if (trig) trig.style.display = 'block';
});

window._testHR = function(bpm) {
  if (window._testHRInterval) { clearInterval(window._testHRInterval); window._testHRInterval = null; }
  if (bpm === null) {
    const pill = document.getElementById('hr-pill');
    if (pill) pill.classList.remove('visible');
    const btn = document.getElementById('hr-sheet-btn');
    if (btn) { btn.textContent = 'Connect Polar'; btn.classList.remove('connected'); }
    updateHRSheetUI();
    closeHRSheet();
    return;
  }
  // Show pill and simulate slight BPM variation for realism
  const pill = document.getElementById('hr-pill');
  if (pill) pill.classList.add('visible');
  updateHRPill(bpm);
  // Simulate ±3 BPM variation every 2s
  window._testHRInterval = setInterval(function() {
    const variation = Math.round((Math.random() - 0.5) * 6);
    updateHRPill(bpm + variation);
  }, 2000);
  closeHRSheet();
};
...
function updateHRFocusOverlay(bpm, zoneKey) {
  ...
  ['zone-resting','zone-recovery','zone-aerobic','zone-anaerobic','zone-vo2max'].forEach(z =>
    overlay.classList.toggle(z, z === (zoneKey || 'zone-resting')));
