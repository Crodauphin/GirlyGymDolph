...
function showTimerAutoToast() {
  ...
  if (!toast) {
    ...
    toast.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);' +
      'background:rgba(232,121,160,0.08);border:1.5px solid #E879A0;border-radius:20px;' +
      'padding:8px 18px;font-family:\'Sofia Sans Condensed\',sans-serif;font-size:12px;font-weight:600;' +
...
function renderModifyBody(record) {
  ...
  dateRow.innerHTML=`<label style="font-size:11px;letter-spacing:1px;color:var(--text-dim);font-family:'Sofia Sans Condensed',sans-serif;font-weight:700;">DATE</label>
    <input type="date" id="modify-date-input" value="${record.date||''}"
      style="background:rgba(232,121,160,.08);border:1px solid var(--border);border-radius:var(--radius-sm);
      padding:6px 10px;color:var(--text);font-family:'Sofia Sans Condensed',sans-serif;font-size:15px;font-weight:600;outline:none;flex:1"/>`;
...
function renderBodyStatsTendency() {
  ...
  function chip(label, trend, invert) {
    ...
    const color = positive ? '#50C88C' : '#E879A0';
    const arrow = val > 0 ? '↑' : '↓';
  ...
  el.innerHTML = `<div style="font-family:'Sofia Sans Condensed',sans-serif;font-size:10px;letter-spacing:1px;color:var(--text-dim);margin-bottom:6px;">LAST 3 MONTHS</div><div>${wChip}${bfChip}${mChip}</div>`;
}
...
function drawDualLineChart(canvas, labels, weights, bfs, tall, muscles) {
  ...
  if(validW.length){
    ...
    ctx.fillStyle='rgba(232,121,160,0.12)'; ctx.fill();
    ctx.beginPath(); ctx.moveTo(xPos(0),yw(weights[0]));
    weights.forEach((v,i)=>ctx.lineTo(xPos(i),yw(v)));
    ctx.strokeStyle='#E879A0'; ctx.lineWidth=2; ctx.lineJoin='round'; ctx.stroke();
    weights.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),yw(v),3,0,Math.PI*2);ctx.fillStyle='#E879A0';ctx.fill();});
    ctx.fillStyle='rgba(157,78,120,0.7)'; ctx.font='10px Exo 2,sans-serif'; ctx.textAlign='right';
    [0,.5,1].forEach(t=>{const v=wMin+wRng*t; ctx.fillText(Math.round(v*10)/10,p.left-4,yw(v)+4);});
    ctx.fillStyle='#E879A0'; ctx.font='10px Exo 2,sans-serif'; ctx.textAlign='left';
    ...
    if(muscles && muscles.some(v=>v!=null)){
      ...
      ctx.strokeStyle='#5B8AE8'; ctx.lineWidth=0.75; ctx.lineJoin='round'; ctx.setLineDash([4,3]); ctx.stroke();
      ctx.setLineDash([]);
      muscles.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),yw(v),2,0,Math.PI*2);ctx.fillStyle='#5B8AE8';ctx.fill();});
    }
  }
  ...
  if(bfs && bfs.some(v=>v!=null)){
    ...
    ctx.strokeStyle='#7EC8E3'; ctx.lineWidth=0.75; ctx.lineJoin='round'; ctx.stroke();
    bfs.forEach((v,i)=>{if(v==null)return;ctx.beginPath();ctx.arc(xPos(i),ybf(v),3,0,Math.PI*2);ctx.fillStyle='#7EC8E3';ctx.fill();});
    ctx.fillStyle='rgba(126,200,227,0.8)'; ctx.font='10px Exo 2,sans-serif'; ctx.textAlign='left';
    [0,.5,1].forEach(t=>{const v=bfMin+bfRng*t; ctx.fillText(Math.round(v*10)/10+'%',W-p.right+4,ybf(v)+4);});
  }


  // X axis labels
  ctx.fillStyle='rgba(157,78,120,0.5)'; ctx.font='10px Exo 2,sans-serif'; ctx.textAlign='center';
  const step=Math.max(1,Math.floor(n/4));
  labels.forEach((l,i)=>{if(i===0||i===n-1||i%step===0) ctx.fillText(l.slice(-5),xPos(i),H-8);});

  // Legend
  ctx.font='10px Sofia Sans Condensed,sans-serif'; ctx.textAlign='left';
  let lx=p.left;
  ctx.fillStyle='#E879A0'; ctx.fillText('● Weight',lx,14); lx+=60;
  if(bfs&&bfs.some(v=>v!=null)){ctx.fillStyle='#7EC8E3'; ctx.fillText('● Body fat',lx,14); lx+=65;}
  if(muscles&&muscles.some(v=>v!=null)){ctx.fillStyle='#5B8AE8'; ctx.fillText('● Muscle mass',lx,14);}
}
...
function renderBodyStatsMetricHistory() {
  ...
  function recentFor(metric, unit, label) {
    ...
    return `<div style="flex:1;min-width:0">
      <div style="font-family:'Sofia Sans Condensed',sans-serif;font-weight:700;font-size:10px;letter-spacing:1px;color:var(--text-dim);margin-bottom:4px">${label}</div>
      ${rows}
    </div>`;
  }
...
function renderProgressChart() {
  ...
  drawLineChart(canvas,points.map(p=>p.date),points.map(p=>p.w),name,'kg','#50C88C','rgba(80,200,140,0.15)');
}
...
function renderTonnageChart() {
  ...
  updateDeloadNotices();
  const colors=weeks.map(w=>w===currentWeek?'#E879A0':'#50C88C');
...
function drawBarChart(canvas,labels,values,title,colors){
  ...
  // Bars
  const colorArr=Array.isArray(colors)?colors:labels.map(()=>colors||'#50C88C');
  values.forEach((v,i)=>{
    if(v===0) return;
    const bx=p.left+i*gap+(gap-bw)/2,bh=v/mx*ch,by=p.top+ch-bh;
    const col=colorArr[i]||'#50C88C';
  ...
  // Title
  ctx.fillStyle='rgba(160,180,224,0.5)';ctx.font='10px Sofia Sans Condensed,sans-serif';ctx.textAlign='left';ctx.fillText(title,p.left,16);
}
function drawLineChart(canvas,labels,values,title,unit,lineColor,fillColor){
  ...
  ctx.textAlign='right';[0,.5,1].forEach(t=>{const v=mn+rng*t;ctx.fillStyle='rgba(157,78,120,0.5)';ctx.fillText(Math.round(v)+unit,p.left-4,y(v)+4);});
  ctx.fillStyle='rgba(160,180,224,0.6)';ctx.font='11px Sofia Sans Condensed,sans-serif';ctx.textAlign='left';ctx.fillText(title,p.left,16);
}
...
function renderGymEquipmentList() {
  ...
  data.forEach((section, catIdx) => {
    ...
    header.innerHTML = `
      <div style="font-family:'Sofia Sans Condensed',sans-serif;font-size:11px;font-weight:700;
        letter-spacing:3px;color:var(--header)">${section.cat.toUpperCase()}</div>
      <button onclick="addEquipmentItem(${catIdx})"
        style="background:rgba(232,121,160,.08);border:1px solid rgba(232,121,160,.25);
        border-radius:20px;color:var(--green2);font-size:12px;padding:3px 10px;
        cursor:pointer;font-family:'Sofia Sans Condensed',sans-serif;font-weight:700;letter-spacing:1px">+ ADD</button>`;
...
function updateHRZoneSettings() {
  ...
  if (table) {
    const rows = [
      ...
      ['Aerobic',    z.aerobic   + '–' + (z.anaerobic-1), '#50C88C'],
      ['Anaerobic',  z.anaerobic + '–' + (z.vo2max-1),    '#F9A8D4'],
    ...
    table.innerHTML = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 16px;margin-top:4px;">' +
      rows.map(([name, range, colour]) =>
        `<span style="color:${colour};font-family:'Sofia Sans Condensed',sans-serif;font-weight:700;font-size:13px;letter-spacing:1px;">${name}</span>` +
        `<span style="color:var(--text-dim);font-family:'Sofia Sans Condensed',sans-serif;font-weight:600;font-size:13px;">${range} bpm</span>`
      ).join('') +
...
function updateHRPill(bpm) {
  ...
  ['zone-resting','zone-recovery','zone-aerobic','zone-anaerobic','zone-vo2max'].forEach(z =>
    pill.classList.toggle(z, z === zoneKey));
  if (!zoneKey) pill.classList.add('zone-resting');
...
function updateHRSheetUI() {
  ...
  if (bpmEl) {
    ...
    const _zc = {'Resting':'rgba(160,180,224,.8)','Recovery':'#7EC8E3','Aerobic':'#50C88C','Anaerobic':'#F9A8D4','VO2 Max':'#FF6B6B'};
    bpmEl.style.color = _zc[zone] || 'var(--text-dim)';
  }
  if (zoneEl) zoneEl.textContent = zone;
  if (zoneEl) { const _zc2 = {'Resting':'rgba(160,180,224,.8)','Recovery':'#7EC8E3','Aerobic':'#50C88C','Anaerobic':'#F9A8D4','VO2 Max':'#FF6B6B'}; zoneEl.style.color = _zc2[zone] || 'var(--text-dim)'; }
...
function updateHRFocusOverlay(bpm, zoneKey) {
  ...
  if (!zoneKey) overlay.classList.add('zone-resting');
